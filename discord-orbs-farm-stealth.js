/*
 * 🔥 DisOrbsFarm v5.4
 * Fixed draggable menu (was stuck to left side)
 */
...
(async () => {
    ...
    function applyStealthPreset(level) {
        ...
        if (level === 1) { CONFIG.VIDEO_BASE_SPEED = 9; CONFIG.VIDEO_MAX_FUTURE = 14; CONFIG.VIDEO_MIN_DELAY = 500; CONFIG.PAUSE_BETWEEN_QUESTS = [5,15]; }
        else if (level === 2) { CONFIG.VIDEO_BASE_SPEED = 4; CONFIG.VIDEO_MAX_FUTURE = 6; CONFIG.VIDEO_MIN_DELAY = 1400; CONFIG.PAUSE_BETWEEN_QUESTS = [45,120]; }
        else { CONFIG.VIDEO_BASE_SPEED = 2; CONFIG.VIDEO_MAX_FUTURE = 3; CONFIG.VIDEO_MIN_DELAY = 2200; CONFIG.PAUSE_BETWEEN_QUESTS = [90,240]; }
    }
    ...
    function playBeep(f=880,d=200){try{const a=new(window.AudioContext||window.webkitAudioContext)();const o=a.createOscillator();const g=a.createGain();o.frequency.value=f;g.gain.value=0.4;o.connect(g);g.connect(a.destination);o.start();setTimeout(()=>{g.gain.exponentialRampToValueAtTime(0.001,a.currentTime+0.1);setTimeout(()=>o.stop(),80);},d);}catch{}}

    function showNotification(title,body){if(!CONFIG.NOTIFICATIONS_ENABLED)return;try{if(Notification.permission==="granted")new Notification(title,{body});else if(Notification.permission!=="denied")Notification.requestPermission().then(p=>{if(p==="granted")new Notification(title,{body});});}catch{}playBeep(660,350);}
    ...
    delete window.$;let wpRequire;try{wpRequire=webpackChunkdiscord_app.push([[Symbol()],{},r=>r]);webpackChunkdiscord_app.pop();}catch{return;}
    ...
    function loadQuests(){
        ...
        raw.forEach(q=>{
            try{
                ...
                const task=tc?.tasks;if(!task)return;
                const type=Object.keys(task).find(k=>k.includes("VIDEO")||k==="PLAY_ON_DESKTOP");if(!type)return;
                const needed=task[type].target||0;
                allAvailable.push({raw:q,id:q.id,name:q.config?.messages?.questName||q.config?.application?.name||"Quest",secondsNeeded:needed,secondsDone:q.userStatus?.progress?.[type]?.value||0,isVideo:type.includes("VIDEO"),isGame:type==="PLAY_ON_DESKTOP",enrolled:!!q.userStatus?.enrolledAt,estimatedOrbs:Math.max(15,Math.floor(needed/18))});
            }catch{}
        });
        allAvailable.sort((a,b)=>(b.enrolled-a.enrolled)||(b.isVideo-a.isVideo));
    }
    loadQuests();if(!allAvailable.length)return;

    const addCleanup=fn=>cleanups.push(fn);
    const runCleanups=()=>{cleanups.forEach(f=>{try{f()}catch{}});cleanups=[]; };

    async function enrollQuest(q){if(q.enrolled)return true;if(!CONFIG.AUTO_ENROLL)return false;for(const loc of[0,1,2,11]){try{const r=await api.post({url:`/quests/${q.id}/enroll`,body:{location:loc}});if(r?.body){q.enrolled=true;return true;}}catch{}await sleep(250);}return false;}
    ...
    function createMainUI(){
        ...
        // FIX: proper initial positioning + draggable
            ...
            <div style="background:rgba(28,28,30,0.93);backdrop-filter:blur(40px)saturate(180%);border:1px solid rgba(255,255,255,0.1);border-radius:18px;padding:14px 16px;width:355px;color:#F5F5F7;font-family:-apple-system,BlinkMacSystemFont,sans-serif;box-shadow:0 20px 60px rgba(0,0,0,0.5);">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;cursor:move;" id="orbs-drag">
                    <div style="display:flex;align-items:center;gap:8px;">
                        ...
                        <div><div style="font-weight:700;font-size:15px;">DisOrbsFarm</div><div style="font-size:9px;color:#8E8E93;">v5.4 • KDStudio</div></div>
                    </div>
                    <div style="display:flex;gap:4px;align-items:center;">
                        <div style="background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.12);border-radius:999px;padding:2px 9px;font-size:11px;display:flex;align-items:center;gap:3px;"><span style="color:#30D158;">💎</span><span id="orbs-count" style="font-weight:600;">0</span></div>
        ...
        dragHandle.onmousedown = (e) => {
            ...
            // Convert from right-positioned to left+top so it can be dragged freely
            ui.style.right = "auto";
    ...
    function createMiniUI(){
        ...
        miniUi.innerHTML=`<div style="display:flex;align-items:center;gap:6px;"><div style="width:18px;height:18px;background:linear-gradient(#5E5CE6,#0A84FF);border-radius:5px;display:flex;align-items:center;justify-content:center;font-size:10px;">👓</div><div style="font-weight:600;font-size:12px;">DisOrbsFarm</div><div style="background:rgba(255,255,255,0.1);border-radius:999px;padding:1px 7px;font-size:10px;display:flex;align-items:center;gap:3px;"><span style="color:#30D158;">💎</span><span id="mini-orbs">0</span></div></div><button id="mini-restore" style="background:#0A84FF;color:white;border:none;border-radius:999px;padding:2px 9px;font-size:10px;font-weight:600;cursor:pointer;">Развернуть</button>`;
        document.body.appendChild(miniUi);
        miniUi.querySelector("#mini-restore").onclick=restoreUI;
    }

    function minimizeUI(){if(!ui)return;isMinimized=true;ui.style.display="none";createMiniUI();const mo=miniUi.querySelector("#mini-orbs");if(mo)mo.textContent=sessionOrbs;}
    function restoreUI(){if(miniUi)miniUi.remove();if(ui)ui.style.display="block";isMinimized=false;}
    ...
    function setupUIListeners(){
        ...
        function renderList(){
            ...
            filtered.forEach(q=>{
                ...
                div.innerHTML=`<input type="checkbox" ${checked} style="accent-color:#0A84FF;width:15px;height:15px;"><div style="flex:1;min-width:0;"><div style="font-weight:600;font-size:11px;display:flex;justify-content:space-between;"><span>${q.isVideo?"🎬":"🎮"} ${q.name}</span><span style="color:#30D158;font-size:10px;">+${q.estimatedOrbs}</span></div><div style="height:2.5px;background:rgba(255,255,255,0.1);border-radius:999px;margin-top:2px;"><div style="height:100%;width:${pct}%;background:linear-gradient(#0A84FF,#5E5CE6);border-radius:999px;"></div></div></div><div style="font-size:9px;color:#8E8E93;min-width:42px;text-align:right;">${Math.floor(q.secondsDone)}/${q.secondsNeeded}s</div>`;
        ...
        extremeChk.onchange=e=>{
            ...
            if(e.target.checked){
                ...
                CONFIG.VIDEO_BASE_SPEED=11;CONFIG.DISABLE_PAUSES=true;CONFIG.AUTO_CLAIM=true;
                speedS.value=11;speedV.textContent="11.0";
    ...
    function updateOrbsDisplay(){if(!ui)return;const el=ui.querySelector("#orbs-count");if(el)el.textContent=sessionOrbs;if(miniUi){const mo=miniUi.querySelector("#mini-orbs");if(mo)mo.textContent=sessionOrbs;}}

    function updateStats(){if(!ui||!sessionStartTime)return;const tEl=ui.querySelector("#stat-time");const qEl=ui.querySelector("#stat-quests");const rEl=ui.querySelector("#stat-orbsrate");if(tEl){const e=Math.floor((now()-sessionStartTime)/1000);tEl.textContent=`${Math.floor(e/60).toString().padStart(2,"0")}:${(e%60).toString().padStart(2,"0")}`;}if(qEl)qEl.textContent=completedCount;if(rEl&&completedCount>0){const min=Math.max(1,(now()-sessionStartTime)/60000);rEl.textContent=Math.floor(sessionOrbs/min);}}

    const updateUI=(status,pct=0)=>{if(!ui)return;const st=ui.querySelector("#orbs-status");const br=ui.querySelector("#orbs-bar");if(st)st.textContent=status;if(br)br.style.width=pct+"%";updateStats();};

    async function startFarm(){
        if(running)return;running=true;stopRequested=false;completedCount=0;sessionOrbs=0;updateOrbsDisplay();sessionStartTime=now();
        let queue=allAvailable.filter(q=>selectedQuestIds.has(q.id));if(queue.length===0)queue=allAvailable.slice(0,5);
        ui.querySelector("#orbs-start").style.display="none";ui.querySelector("#orbs-stop").style.display="block";

        for(let i=0;i<queue.length;i++){
            if(stopRequested)break;const q=queue[i];await enrollQuest(q);
            try{
                if(q.isVideo){
                    ...
                    while(done<needed&&!stopRequested){
                        ...
                        done+=sp;q.secondsDone=done;updateUI(`🎬 ${q.name}`,Math.floor((done/needed)*100));
                    }
                }else if(q.isGame){await sleep(Math.min(q.secondsNeeded*700,180000));}
                completedCount++;const earned=q.estimatedOrbs||Math.floor(25+rnd(5,15));sessionOrbs+=earned;updateOrbsDisplay();updateStats();
            }catch(e){log("Ошибка: "+e.message,"error");}
            if(!CONFIG.DISABLE_PAUSES&&i<queue.length-1&&!stopRequested){const p=CONFIG.EXTREME_MODE?rndInt(2,8):rndInt(...CONFIG.PAUSE_BETWEEN_QUESTS);for(let s=p;s>0&&!stopRequested;s--){updateUI(`Пауза ${s}с`,0);await sleep(1000);}}
        }
        running=false;ui.querySelector("#orbs-start").style.display="block";ui.querySelector("#orbs-stop").style.display="none";updateUI("Готов",100);
        const msg=`Сессия завершена • ${completedCount} квестов • ${sessionOrbs} орбов`;log(msg,"success");showNotification("DisOrbsFarm",msg);
    }

    if(CONFIG.SHOW_UI){createMainUI();log("DisOrbsFarm v5.4 готова (drag исправлен)","success");}
